		...
		<nav id="site-navigation" aria-label="<?php esc_attr_e( 'Main Navigation', 'plement' ); ?>">
			...
			<ul class="hidden h-[5rem] w-full items-center lg:flex">
				...
				foreach ( $items as $item ) :
					...
					?>
					<li x-data="{open: false}" class="border-r-textSecondary h-full w-full border-r font-semibold">
						<div class="h-full" @mouseover='open = true' @mouseover.away="open = false">
							<a href="<?php echo esc_url( $item['url'] ); ?>"
								class="group flex items-center justify-center h-full w-full text-bodyRegular group  transition-colors duration-300 <?php echo $is_contact_us ? 'hover:bg-accent text-accent !text-bodyBold gap-2' : '' ?> <?php echo $has_children || $is_contact_us ? 'scroll-to hover:bg-accent' : 'hover:bg-[#CBCBCC]' ?>">
								<span
									class=" <?php echo $has_children || $is_contact_us ? 'group-hover:text-white' : '' ?>">
									<?php echo esc_html( $item['title'] ); ?>
								</span>
								<?php if ( $has_children ) : ?>
									<svg :class="open && 'rotate-180 text-white'" width="17" height="16" viewBox="0 0 17 16"
