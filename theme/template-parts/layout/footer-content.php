...
function mobile_menu()
{
	...
	$menu_id        = $menu_locations['menu-3'];

	$items = plmt_menu_builder($menu_id);
	?>
	<nav class="mb-[3.75rem] lg:hidden">
		<ul class="text-center space-y-10">
			<?php foreach ($items as $top_level_item):
				$has_children = $top_level_item['children'];
				?>
				<?php if ($has_children): ?>
					<?php mobile_menu_dropdown($top_level_item['title'], $top_level_item['children']); ?>
				<?php else: ?>
					<li class="text-button">
						<a href="<?php echo $top_level_item['url'] ?>">
							<?php echo esc_html($top_level_item['title']) ?>
						</a>
					</li>
				<?php endif; ?>
			<?php endforeach; ?>
...
function menu_colunn()
{
	...
	<?php if (has_nav_menu('menu-2')): ?>
		<div class="hidden lg:grid grid-flow-col gap-16">
			<?php foreach ($items as $top_level_item): ?>
				<div>
					<span class="hidden lg:inline-block uppercase text-textSecondary font-bold mb-8 leading-[1rem]">
						<?php echo esc_html($top_level_item['title']); ?>
					</span>
