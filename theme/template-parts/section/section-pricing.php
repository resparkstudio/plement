...
function pricing_header( $pricing_content ) {
	...
	<div class="container relative flex flex-col items-center justify-center text-center mb-8 md:mb-2">
		...
		<div class="flex justify-center max-w-[20rem] m-auto w-full mb-4 md:mb-5">
			<div class="relative flex w-full p-1 bg-white rounded-full">
				<span class="absolute inset-0 m-1 pointer-events-none" aria-hidden="true">
					<span
						class="absolute inset-0 w-1/2 bg-textBlack rounded-full shadow-sm shadow-indigo-950/10 transform transition-transform duration-150 ease-in-out"
				...
				<button
					...
					:class="isAnnual ? 'text-white' : 'text-slate-500'" @click="isAnnual = true" :aria-pressed="isAnnual">
					<?php esc_html_e( 'Packages', 'plmt' ) ?>
				</button>
				<button
					class="relative flex-1 text-sm font-medium h-8 rounded-full focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300 transition-colors duration-150 ease-in-out"
					:class="isAnnual ? 'text-slate-500' : 'text-white'" @click="isAnnual = false" :aria-pressed="isAnnual">
